...
def seed_ativos_eu():
    ...
    with app.app_context():
        ...
        for ticker, nome, bolsa, setor, preco in stocks_intl:
            if Ativo.query.filter_by(ticker=ticker, mercado='EU').first():  # FIX M2-ATIVOS-005
                print(f"  ⚠️  {ticker} - existe (EU), skipped")
                skipped += 1
                continue
            ativo = Ativo(
        ...
        for ticker, nome, bolsa, cat, preco in etfs_intl:
            ...
            ativo = Ativo(
                ticker=ticker, nome=nome,
                tipo=TipoAtivo.ETF_INTL,
                classe=ClasseAtivo.RENDA_VARIAVEL,
                mercado='EU', moeda='EUR',
                preco_atual=preco,
                observacoes=f"Bolsa: {bolsa} | Categoria: {cat}",
                ativo=True
            )
            db.session.add(ativo)
            created += 1
