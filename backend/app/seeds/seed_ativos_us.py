...
# Corrigido M2-ATIVOS-005: checagem de existência usa par (ticker, mercado)
# para garantir idempotência e alinhamento com UNIQUE (ticker, mercado)
...
def seed_ativos_us():
    ...
    with app.app_context():
        ...
        for ticker, nome, setor, preco in stocks:
            if Ativo.query.filter_by(ticker=ticker, mercado='US').first():  # FIX M2-ATIVOS-005
                print(f"  ⚠️  {ticker} - existe (US), skipped")
                skipped += 1
                continue
            ativo = Ativo(
